...
## Fedora 42 KDE 兼容性修复
本项目已针对 Fedora 42 KDE 的 Wayland 默认环境进行了优化：

### 修复的问题
- **Wayland 兼容性**: 自动检测 Wayland 环境并回退到 X11 后端
- **输入法框架**: 增强了 fcitx 在混合显示环境下的稳定性
- **权限问题**: 改进了容器内的用户/组映射处理

### 使用建议
在 Fedora 42 KDE 下使用时，建议添加以下环境变量：
```bash
-e GDK_BACKEND=x11
-e QT_QPA_PLATFORM=xcb
```

如果遇到权限问题，请确保数据目录权限正确：
```bash
